...
export function generateColorClasses(numberOfColors: number, baseIntensityIndex: number): string[] {
  const colors: string[] = [];
  ...
  for (let i = 0; i < numberOfColors; i++) {
    ...
    if (i >= tailwindColors.length) {
      ...
      currentIntensityIndex = baseIntensityIndex === intensities.length - 1
        ? baseIntensityIndex
        : Math.min(baseIntensityIndex + cycleNumber, intensities.length - 1);
    }

    const colorIndex = i % tailwindColors.length;
    colors.push(`${tailwindColors[colorIndex]}-${intensities[currentIntensityIndex]}`);
  }
